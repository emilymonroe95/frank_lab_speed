...
def get_electrode_ids(nwb_file_name, which_elects, ref):
    #takes nwb file name(str), which_elects = every ___th electrode (int). For example, if you set it to be 4, it will grab every 4th electrode. 
    ...
    #get all electrode ids fom electrode table
    ...
    #if you want the reference to be added, check which electrode is the ref and add it. 
    if ref == True:
...
def remove_dead_tet_chans(nwb_file_name, lfp_electrode_ids, perm_dead_chan):
    #this function takes nwb file, current list of electrode ids, and perm_dead_channels which is the channel number above which all tetrode channels should be dead. (by perm. dead, i mean the ones above tetrode 24 but below 32 for us)
...
def checking_other_dead_tets(nwb_file_name,lfp_electrode_ids): 
      #this cell will check if there are any dead tetrodes in the lfp_electrodes list. if so, it will remove them, and add the next electrode to the list if it isnt dead. returns modified list of electrode ids, at this point with no dead channels included. 
        ...
        #very hacky way to do this.. exlucing anything permanantly dead. (because we dont want to add 25.2 if 25.1 is dead, for example.)
...
def get_timestamps_and_data(nwb_file_name, filter_type: List=None, data_type: List=None):
    ...
    #filter type is a list ofstrings- can be "theta" or "ripple"
    
    #eg: lfp_timestamps, lfp_data, lfp_eseries = get_timestamps_and_data(nwb_file_name,data_type = ['lfp'])
...
def get_speed(position_info,epoch,time_from_start, time_interval_s):
    # position_info is the result of calling PositionInfo(key). 
    
    #epoch is the VALID TIMES from the wanted epoch. Calculated with: 
    # epoch = (IntervalList & {'nwb_file_name':nwb_file_name} & {'interval_list_name':'02_r1'}).fetch1('valid_times')
    
    #time from start is s from the start of position data, time interval is the total number of seconds you want. (ints)
    #this function will just mask the position data and will return head speeds from that masked time. 
...
def get_x_y_list(time_from_start, time_interval_s,epoch, electrode_id, eseries, timestamps, data):
    #from a list of electrodes, in the context of plotting lfp signals, make the data using times (Not timestamps) 
    #use ints - time from start(s) and amount of time (time_interval_s = (s)). 
    #EPOCH IS VALID TIMES from the epoch. 
    # epoch = (IntervalList & {'nwb_file_name':nwb_file_name} & {'interval_list_name':'02_r1'}).fetch1('valid_times')
...
def simple_plot(x,y,title,xlab,ylab):
    #this was my fractice function to make a simple plot and label axis, but it didnt really save energy (too many inputs) so i dont think i used.
    f=plt.figure()
    plt.plot(x,y)
    plt.title(title)
    plt.xlabel(xlab)
...
def plot_overlay(x0,y0,lab0,color0,x1,y1,lab1,color1,title,xlab,ylab, offset= None, x2=None,y2=None,lab2=None,color2=None, x3=None,y3=None,lab3=None,color3=None,x4=None,y4=None,lab4=None,color4=None): 
    #this takes 2 signals (x and y) (optional 4 signals) and plots them with axis labels, optional offset (if none, will be overlayed)
...
def plot_overlay_with_pos(lfp_eseries, orig_eseries, theta_eseries, ripple_eseries, position_info, nwb_file_name, time_from_start, time_interval_s, epoch, electrode_id,offset):
    #plot of all four data types and then speed on a lower subplot. just pass in the eseries for each signal, position info (which has the speed data) , the nwb file name for labeling, the time from start/time in seconds desired, the electrode channel(s) as a list, and an offset in uV if desired. 
...
def get_speed_timestamp(start_timestamp, end_timestamp, position_info):
    #this function does the same thing as get_speed but takes a specific timestamp instead, and should be condensed with the other function tomorrow
...
def get_pos_timestamp(start_timestamp, end_timestamp, position_info):
    #this function does the same thing as get_pos but takes a specific timestamp instead, and should be condensed with the other function tomorrow
...
def find_overlapping_times(data_list, interval_list=None):
    ...
    #use this function in the context of ripple filtering. written by rhino. 
...
def ripple_detector(nwb_file_name, pos_interval, tetrode_ind): 
    #this will do everything to get ripple times.. take string of the interval to use (eg "pos 2 valid times" )
    #and i guess a lsit of which tetrodes you want. i would reccomend trying this outside the context of the function to see how it works first. 
